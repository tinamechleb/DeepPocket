...
import PetitIncome from './PetitIncome';
import CategoriesFormEdit from '../PopupFroms/CategoriesFormEdit';


class GrandIncomeCategoriesCard extends Component {
    ...
    componentWillMount() {
        fetch(`http://127.0.0.1:8000/api/sum/categories/income/${this.props.id}`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`,
                'Accept': `application/json`
            ...
            .then(data => {
                this.setState({ sum: data }); console.log(data);
            })
            .catch(err => {
                console.log(err);
            });;
                    ...
                    <Accordion fluid styled>
                        ...
                        <Accordion.Content active={activeIndex === 1}>
                            <div className="surprise petit-surprise">
                                <PetitIncome color={this.props.color} id={this.props.id} amount={this.state.sum} currency={this.props.currency} />
                            </div>
                        </Accordion.Content>
                    </Accordion>
...
export default GrandIncomeCategoriesCard;
